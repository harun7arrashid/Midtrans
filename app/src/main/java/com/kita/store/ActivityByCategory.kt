...
class ActivityByCategory : AppCompatActivity() {
    ...
    private fun getProduct(){
        ...
        var reques = JsonArrayRequest(Request.Method.GET, "https://store.pangperhebat.my.id/apiproductid.php?cat_id="+GlobalData.idCategory, null, Response.Listener { response ->
            for (s in 0..response.length() - 1){
                ...
                var photo = job.getString("photo").replace("localhost", "store.pangperhebat.my.id")
